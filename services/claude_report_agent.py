...
class SQLQueryAnalyzer:
    ...
    def initialize_components(self):
        try:
            ...
            self.llm = ChatAnthropic(temperature=0.2, model="claude-sonnet-4-20250514")
    ...
    def generate_performance_report(self, context: str) -> str:
        try:
            plan_analysis = ""
            if "### MySQL Execution Plan:" in context:
                plan_start = context.find("### MySQL Execution Plan:")
                plan_end = context.find("###", plan_start + 1)
                if plan_end == -1:
                    plan_content = context[plan_start:]
                else:
                    plan_content = context[plan_start:plan_end]

                plan_tree = plan_content.replace(
                    "### MySQL Execution Plan:", ""
                ).strip()
                if plan_tree and plan_tree != "```\nNo execution plan available\n```":
                    plan_analysis = self.analyze_execution_plan_detailed(plan_tree)

            report_prompt = f"""
            You are an expert SQL Performance Analysis Agent specializing in MySQL query optimization.
            ...
            **IMPORTANT**: 
            ...
            {plan_analysis if plan_analysis else 'No execution plan analysis available.'}
            
            Please generate a comprehensive performance analysis report following the structure above.
            """
    ...
    def generate_query_rewrite(self, context: str, performance_report: str) -> str:
        """Generate query optimization recommendations using direct LLM call"""
        try:

            rewrite_prompt = f"""
            You are an expert SQL Query Optimization Agent specializing in MySQL query rewrites.
            ...
            **IMPORTANT**: 
            ...
            Please generate optimized query versions with detailed explanations and schema-aware improvements.
            """
            ...
            return f"Error generating query rewrite: {str(e)}"
            ...
            try:
                ...
                **Schema Context for Optimization:**
                ...
                query_rewrite = self.generate_query_rewrite(
                    rewrite_context, state.get("performance_report", "")
                )
    ...
    def generate_comprehensive_analysis(
        ...
        try:
            ...
            # print("🚀 Starting SQL query analysis with Claude Sonnet 4 and simple hallucination detection...")
    ...
    def analyze_from_json_input(self, json_input: dict, schema_info: dict = None):
        return self.generate_comprehensive_analysis(json_input, schema_info)


def run_claude_report_generation(session_id: str):
